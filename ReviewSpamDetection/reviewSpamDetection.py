...
def dataPreProcessingComplete(content):
    ...
    #return dataPreProcessingPOS(content)
    #return  dataPreProcessing(content)+" "+dataPreProcessingPOS(content)      
def trainClassifier(folderNameTrue,folderNameFake,classifier,parameters,ngramRange):
    ...
    train_data_featuresTfIDF = TfidfTransform.fit_transform(train_data_features)
    # Numpy arrays are easy to work with, so convert the result to an 
    # array
    train_data_features = train_data_featuresTfIDF.toarray()
    ...
    return (clf,vectorizer,TfidfTransform)
def testClassifierAccuracy(clf,vectorizer,TfidfTransform,folderNameTrueTest,folderNameFakeTest):
    #Getting Test Data    
    #fileName="t_hilton_1.txt"
    testTrue_X=[]
    ...
    test_data_featuresIDF=TfidfTransform.transform(test_data_features)
    test_data_features = test_data_featuresIDF.toarray()
...
#parameters = {'alpha': (0.001,0.0001,0.00001, 0.000001,0.0000001),
...
ngramRange=(1, 1)
clf,vectorizer,TfidfTransform=trainClassifier(folderNameTrue,folderNameFake,classifier,parameters,ngramRange)
...
testClassifierAccuracy(clf,vectorizer,TfidfTransform,folderNameTrueTest,folderNameFakeTest)
