...
// <<< Don't Change
    ...
    public static class TitleCountMap extends Mapper<Object, Text, Text, IntWritable> {
        ...
        public void map(Object key, Text value, Context context) throws IOException, InterruptedException {
        	...
        	String nextTokenLowerCase=nextToken.trim().toLowerCase();
        	if (!stopWords.contains(nextTokenLowerCase)) {
        	context.write(new Text(nextTokenLowerCase), new IntWritable(1));
        	}
        }
        }
    }
    ...
    public static class TopTitlesStatReduce extends Reducer<NullWritable, TextArrayWritable, Text, IntWritable> {
        ...
        public void reduce(NullWritable key, Iterable<TextArrayWritable> values, Context context) throws IOException, InterruptedException {
            ...
            Integer [] valueArr=new Integer[N];
            // TODO
        ...
        private Integer mean(Integer[] arr)
        {
            double meanDouble=0.0;
            meanDouble=sum(arr)/arr.length;
            Integer mean=0;
            mean=(int)Math.floor(meanDouble);
            return mean;
        }
        private Integer variance(Integer [] arr)
        {
            Integer meanArr=mean(arr);
           Integer var = 0;
           for (Integer n : arr)
           {
               var += (n-meanArr)*(n-meanArr);
           }
           var=(int)Math.floor(var/arr.length);
            return var;
        }
    }
